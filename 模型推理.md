...
```shell
cd /home/edgeboard
wget https://bj.bcebos.com/ppdeploy/ppdeploy1.1/SDK/PPDeploy1.1_resnet_PaddleClas2.5_Paddle2.4.1_Ver1.0.0_c++.zip
```

在终端中输入以下命令，解压文件。

```shell
unzip PPDeploy1.1_resnet_PaddleClas2.5_Paddle2.4.1_Ver1.0.0_c++.zip
```

Step：在终端中输入以下命令，进入resnet-c++目录，编译c++文件。

```shell
cd /home/edgeboard/resnet-c++
mkdir build
cd build
cmake ..
make -j4
